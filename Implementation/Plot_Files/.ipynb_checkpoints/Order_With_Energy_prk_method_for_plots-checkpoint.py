...
def find_error(A1D, H_sequence, stp):
    ...
    j1_iyn_list = iyn_list[9:total_isteps*NN:isteps]
    j2_izn_list = izn_list[9:total_isteps*NN:isteps]
    ...
    ## Mean Squared Error
    final_error = (jnp.sum(jnp.square(err1)) + jnp.sum(jnp.square(err2))) / (2 * NN)
    ...
    ### no regularizer here :
    return jnp.sum(final_error), energy_error, yn_list, zn_list, H #, step_size_list_convergence, o_error_list_convergence
