...
import withoutE_prk_method.Random_prk_for_optimization as IRK4
import Important_functions.Convert_1D2D as convert


def main():
    output_path = 'Without_Energy/One-Random-Outputs/'
    ...
    InitalWeight_path= f'{output_path}Initial_weights.txt'

    # Initial weights 
    ## Lobatto 3A and B fourth order
    key = jax.random.PRNGKey(0)

    """ Initial weight selections
    """
    # # Initialize random weights for A1 and B1
    A1 = jax.random.uniform(key, (4, 4), minval=0.0, maxval=1.0)
    B1 = jax.random.uniform(key, (4,), minval=0.0, maxval=1.0)

    # # Lobatto 3B
    A2 = jax.random.uniform(key, (4, 4), minval=0.0, maxval=1.0)
    B2 = jax.random.uniform(key, (4,), minval=0.0, maxval=1.0)
    ...
    initial_A1D = convert.Convert_toOneD(A1, A2, B1, B2)
    ...
    compute_grads_batched = jax.pmap(numerical_gradient, in_axes=(None, 0), axis_name='batch')  # pmap instead of vmap
    ...
    with open(Validation_path, 'w') as f:
        f.write("")
    with open(FinalWeight_path, 'w') as f:
        f.write("")

    with open(InitalWeight_path, 'w') as f:
            f.write("")
        

    """ Saving initial weights A1D beofre optimization. Since we are using random numbers saving is need to recreate the results for before optimization and after optimization
    """
    initial_jnp_array_A1D = jnp.array(initial_A1D, dtype=jnp.float64)
    initial_np_array_A1D = np.array(initial_jnp_array_A1D, dtype=np.float64)
    with open(InitalWeight_path, 'w') as Initial_weights:
        np.savetxt(Initial_weights, initial_np_array_A1D, delimiter=',', fmt='%.18e')
        Initial_weights.flush()
        

    analytical_errors_list = []
    energy_errors_list = []
    validation_errors_list = []
    switch = True # Swtiching from ADAM to SGD
