...
def find_error(A1D, H_sequence):
    ...
    # yn_list = yn_list.at[0].set(H_sequence[4])
    # zn_list = zn_list.at[0].set(H_sequence[5])
    # yn_list = jnp.concatenate(y0, yn_list)
    yn_list = jnp.concatenate([y0, yn_list])  # Wrap y0 and yn_list in a list

    # zn_list = jnp.concatenate(z0, zn_list)
    zn_list = jnp.concatenate([z0, zn_list])  # Wrap y0 and yn_list in a list
    ...
    return y0, z0, yn_list, zn_list, H[0] , H, energy_error  #, step_size_list_convergence, o_error_list_convergence
