...
def find_error(A1D, H_sequence):
    ...
    # yn_list = jnp.concatenate([y0, yn_list])
    # zn_list = jnp.concatenate([z0, zn_list])
    ### Adding the first values in the list , i.e. y0 and z0 to their respective yn_list and zn_list and then checking the result. 
    e_yn_list = jnp.concatenate([y0, yn_list])
    e_zn_list = jnp.concatenate([z0, zn_list])
    H = Energy_Function(e_yn_list, e_zn_list, alpha_values) # H should be of type list

    # H = Energy_Function(yn_list, zn_list, alpha_values) # H should be of type list
    
    # print("############################# Length of H = ==================" , len(H))
    energy_error = jnp.sum(jnp.square(H - H[0])) / (len(H)-1) # Hamiltonian Error
